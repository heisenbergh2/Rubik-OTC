...
Services = {
    ...
    gamenews = "http://127.0.0.1/game_announcements.php", --./game_announcements
}
...
g_app.setName("RubikOTC");
g_app.setCompactName("RubikOTC");
g_app.setOrganizationName("RubikOTC");
...
-- ceiling here so the 1530 protocol (our server + assets/things/1530) is accepted.
...
-- Try to add on-disk assets/, modules/, mods/ to the search path.
...
g_resources.addSearchPath(g_resources.getWorkDir() .. 'assets', true)
g_resources.addSearchPath(g_resources.getWorkDir() .. 'modules', true)

-- These CSS files are optional; the client's OTUI styles work without them.
for _, cssFile in ipairs({ '/styles/html.css', '/styles/custom.css' }) do
    if g_resources.fileExists(cssFile) then
        g_html.addGlobalStyle(cssFile)
    end
end
