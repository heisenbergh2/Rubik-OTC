...
The OTML parser supports `&` definitions, root-level `$name: value` palette definitions, and `$name` references in values. Root-level dollar definitions allow palettes such as `assets/styles/0-vars.otui` to keep the client-nine syntax. Nested `$pressed:` and similar widget states remain style selectors, not variable definitions. This document explains the behavior, scope rules, and important considerations.
...
```otui
&variableName: value
$paletteVariable: value  // root level only
```
