...
TEST(OTMLAlias, ResolvesDollarPaletteAcrossDocumentsWithoutConsumingWidgetStates)
{
    std::istringstream palette("$var-test-palette-color: #A1B2C3\n");
    const auto paletteDoc = OTMLDocument::parse(palette, "palette.otui");
    EXPECT_EQ("#A1B2C3", paletteDoc->globalAliases().at("var-test-palette-color"));

    std::istringstream style(R"(
TestStyle < UIWidget
  color: $var-test-palette-color
  $pressed:
    color: #FFFFFF
)");
    const auto styleDoc = OTMLDocument::parse(style, "style.otui");
    const auto widget = findStyleByTag(styleDoc, "TestStyle < UIWidget");
    ASSERT_NE(nullptr, widget);
    EXPECT_EQ("#A1B2C3", widget->valueAt("color"));
    EXPECT_NE(nullptr, widget->get("$pressed"));
}
