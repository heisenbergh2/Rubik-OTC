...
local function measureDetailRowHeight(value, valueWidth)
	...
	detailMeasureLabel:setFont("Verdana Bold-11px-new")
...
local function getCharacterSelectedItemTier()
	if not selectedCharacterSlot or not tibiaInspectCharacter then
		return 0
	end
...
local function prepareCharacterItemDescriptionRows(descriptions)
	...
	if hasClassification then
		local tierRow = {
			tr("Tier"),
			tostring(getCharacterSelectedItemTier())
		}
...
local function updateCharacterInspectImbuements(entry)
	...
	for index, iconId in ipairs(imbuements) do
		...
		if widget then
			...
			if resource then
				if iconId > 0 then
					resource:setImageClip(string.format("%d 0 64 64", iconId * 64))
				else
					resource:setImageClip("0 0 64 64")
				end
			end
...
local function renderCharacterPlayerDetails()
	if not tibiaInspectCharacter or not characterInspectData then
		return
	end
...
local function applyCharacterOutfitWidget()
	if not tibiaInspectCharacter or not characterInspectionOutfit then
		return
	end
...
local function onCharacterInventorySlotClick(slot, widget)
	if not tibiaInspectCharacter or not tibiaInspectCharacter:isVisible() then
		return
	end
...
local function buildCharacterInspectCopyText()
	...
	local lines = {
		string.format("You are inspecting: %s", inspectName)
	}
...
function openSelfInspectCyclopedia()
	if not isCharacterSelfInspect() then
		return
	end
...
function openSelfInspectProficiency()
	if not isCharacterSelfInspect() or not selectedCharacterSlot then
		return
	end
...
function toggleCharacterView(widget)
	if not tibiaInspectCharacter or not widget then
		return
	end
...
function returnToCharacterInspect()
	if not characterInspectReturnData or not characterInspectReturnData.data then
		return false
	end
...
function showCharacter(data)
	if not tibiaInspectCharacter or not data then
		return
	end
...
local function onParseCyclopediaCharacterInspection(data)
	...
	if not characterInspectPendingOpen then
		return
	end
...
local function buildInspectCopyText()
	local lines = {
		string.format("You are inspecting: %s", inspectedItemName or "")
	}
...
function openProficiency()
	...
	if not proficiencyMod or not proficiencyMod.requestOpenWindow then
		return
	end
...
function onInspection(inspectType, itemName, item, descriptions, imbuements)
	...
	for index, iconId in ipairs(imbuements) do
		local slotId = slotByPosition[3 - slotCount + index]
