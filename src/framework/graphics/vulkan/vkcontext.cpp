...
bool VkContext::init(void* windowHandle, const int width, const int height)
{
    ...
    g_logger.info("[vulkan] context ready: {} | swapchain {}x{}, images: {}, frames in flight: {}",
                  m_deviceName, m_swapchainExtent.width, m_swapchainExtent.height,
                  m_swapchainImages.size(), MAX_FRAMES_IN_FLIGHT);
    ...
    if (!m_batch.init(m_physicalDevice, m_device, m_renderPass, m_graphicsQueue, m_commandPool,
        ...
        g_logger.info("[vulkan] sprite batch unavailable - falling back to the single stage 2 rectangle");
...
bool VkContext::pickPhysicalDevice()
{
    ...
    g_logger.info("[vulkan] selected device: {} (queues: graphics {}, presentation {})",
                  m_deviceName, m_graphicsFamily, m_presentFamily);
