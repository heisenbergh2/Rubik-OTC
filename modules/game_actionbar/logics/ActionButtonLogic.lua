...
local function buttonIsEmpty(button)
	return button.item:getItemId() == 0 and string_empty(button.item.text:getText()) and string_empty(button.item.text:getImageSource())
end
...
local function bindHotkey(button, hotkey)
	...
	g_keyboard.bindKeyPress(combo, function()
		if not HotkeyUtils.canPerformKeyCombo(combo) then
			return
		end
...
local function setupHotkeyButton(button)
	...
	for _, data in pairs(ApiJson.getHotkeyEntries(chatMode)) do
		if data.actionsetting and data.actionsetting.action == "TriggerActionButton_" .. button:getId() then
...
local function translateDisplayHotkey(text)
	...
	local display = g_keyboard and g_keyboard.formatHotkeyDisplayText(text) or text

	if g_keyboard and g_keyboard.truncateHotkeySlotText then
		return g_keyboard.truncateHotkeySlotText(display)
	end

	if #display > 5 then
		return "..." .. display:sub(-3)
	end

	return display
end

local function formatHotkeyTooltipText(text)
	if not text or text == "" then
		return "None"
	end

	if g_keyboard and g_keyboard.formatHotkeyTooltipText then
		local tooltip = g_keyboard.formatHotkeyTooltipText(text)
...
function checkRemainSpellCooldown(button, spellId)
	if not modules.client_options.getOption("graphicalCooldown") and not modules.client_options.getOption("cooldownSecond") then
		return true
	end
...
function configureButtonMouseRelease(button)
	function button.onMouseRelease(button, mousePos, mouseButton)
		button.cache = getButtonCache(button)
		...
		if mouseButton == MouseRightButton then
			...
			menu:addOption(button.cache.isPassive and tr("Edit Passive Ability") or tr("Assign Passive Ability"), function()
			...
			if button.item and button.item:getItemId() > 100 and modules.game_bot then
				menu:addSeparator()

				local useThingId = button.item:getItemId()

				menu:addOption("ID: " .. useThingId, function()
					g_window.setClipboardText(useThingId)
				end)
			end
...
function updateButton(button)
	...
	function button.item:onDragMove(mousePos)
		if not button.cache.dragPrepared then
			...
			button:setParent(gameRootPanel)
			button:breakAnchors()
		end

		button:setVisible(true)
		button:raise()
		button:setX(mousePos.x - 16)
		button:setY(mousePos.y - 16)
	...
	function button.item:onDragLeave(widget, mousePos)
		...
		button.cache.dragPrepared = nil

		onDragItemLeave(button, mousePos, button)
	...
	function button.item.text.onClick()
		onExecuteAction(button)
	end
...
function onDragItemLeave(self, mousePos, button)
	...
	if button.cache.actionType == UseTypes.chatText then
		ApiJson.createOrUpdateText(tonumber(destBarID), tonumber(destButtonID), button.cache.param, button.cache.sendAutomatic)
	elseif itemId ~= 0 then
		ApiJson.createOrUpdateAction(tonumber(destBarID), tonumber(destButtonID), getActionName(button.cache.actionType), itemId, button.cache.upgradeTier)
	elseif button.cache.isPassive then
	...
	if isButtonEmpty then
		...
		if destButtonCache.actionType == UseTypes.chatText then
			ApiJson.createOrUpdateText(tonumber(draggedBarID), tonumber(draggedButtonID), destButtonCache.param, destButtonCache.sendAutomatic)
		elseif destButtonCache.itemId ~= 0 then
			ApiJson.createOrUpdateAction(tonumber(draggedBarID), tonumber(draggedButtonID), getActionName(destButtonCache.actionType), destButtonCache.itemId, destButtonCache.upgradeTier)
