...
function Fusion:proceedFusion()
	...
	g_game.sendForgeAction(0, self.convergenceWidget:isChecked(), self.leftItemId, self.fusionTier, self.rightItemId, self.improveClicked == true, self.tierLossClicked == true)
end
...
function Fusion:displayItems()
	...
	for i, v in ipairs(self.data.fusionItems) do
		...
		container, _, capturedItem = createFusionListItem(self.contentPanel, id, v.tier, v.count, i, function()
			...
			local cost = self.classificationTable[tostring(nextItem:getClassification())][tostring(v.tier)]
