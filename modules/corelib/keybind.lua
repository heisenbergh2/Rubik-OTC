...
local function bindComboKeyDown(combo, callback, widget, alone)
	if isMouseHotkeyCombo(combo) then
		g_mouse.bindHotkeyPress(combo, callback, widget)
	else
		g_keyboard.bindKeyDown(combo, callback, widget, alone)
	end
end

local function bindComboKeyUp(combo, callback, widget, alone)
	if isMouseHotkeyCombo(combo) then
		return
	end

	g_keyboard.bindKeyUp(combo, callback, widget, alone)
end
...
local function unbindComboKeyUp(combo, callback, widget)
	if isMouseHotkeyCombo(combo) then
		return
	end

	g_keyboard.unbindKeyUp(combo, callback, widget)
end
...
function Keybind.init()
	...
	if g_settings.getBoolean("cip_import_skip_session_save") then
		g_settings.remove("cip_import_skip_session_save")
	end
	...
	if not Keybind.currentPreset or Keybind.currentPreset == "" or not Keybind.presetToIndex[Keybind.currentPreset] then
...
function Keybind.new(category, action, primary, secondary, alone)
	...
	if type(primary) == "string" then
		...
		keys[CHAT_MODE.ON] = {
			primary = primary[CHAT_MODE.ON]
		}
		keys[CHAT_MODE.OFF] = {
			primary = primary[CHAT_MODE.OFF]
		}
	end
	...
	if Keybind.defaultKeys[CHAT_MODE.ON][keys[CHAT_MODE.ON].primary] then
		...
		perror(string.format("Default primary key (Chat Mode On) assigned to [%s: %s] is already in use by [%s: %s]", category, action, primaryKeybind.category, primaryKeybind.action))
	...
	if Keybind.defaultKeys[CHAT_MODE.OFF][keys[CHAT_MODE.OFF].primary] then
		...
		perror(string.format("Default primary key (Chat Mode Off) assigned to [%s: %s] is already in use by [%s: %s]", category, action, primaryKeybind.category, primaryKeybind.action))
	...
	if keys[CHAT_MODE.ON].secondary and Keybind.defaultKeys[CHAT_MODE.ON][keys[CHAT_MODE.ON].secondary] then
		...
		perror(string.format("Default secondary key (Chat Mode On) assigned to [%s: %s] is already in use by [%s: %s]", category, action, secondaryKeybind.category, secondaryKeybind.action))
	...
	if keys[CHAT_MODE.OFF].secondary and Keybind.defaultKeys[CHAT_MODE.OFF][keys[CHAT_MODE.OFF].secondary] then
		...
		perror(string.format("Default secondary key (Chat Mode Off) assigned to [%s: %s] is already in use by [%s: %s]", category, action, secondaryKeybind.category, secondaryKeybind.action))
	...
	for _, callback in ipairs(keybind.callbacks) do
		if callback.type == KEY_UP then
			if keys.primary then
				...
				if keys.primary:len() > 0 then
					bindComboKeyUp(keys.primary, callback.callback, keybind.widget, callback.alone)
				end
			end
			...
			if keys.secondary then
				...
				if keys.secondary:len() > 0 then
					bindComboKeyUp(keys.secondary, callback.callback, keybind.widget, callback.alone)
				end
			end
		elseif callback.type == KEY_DOWN then
			if keys.primary then
				...
				if keys.primary:len() > 0 then
					bindComboKeyDown(keys.primary, callback.callback, keybind.widget, callback.alone)
				end
			end
			...
			if keys.secondary then
				...
				if keys.secondary:len() > 0 then
					bindComboKeyDown(keys.secondary, callback.callback, keybind.widget, callback.alone)
				end
			end
		elseif callback.type == KEY_PRESS then
			if keys.primary then
				...
				if keys.primary:len() > 0 then
					bindComboKeyPress(keys.primary, callback.callback, keybind.widget)
				end
			end
			...
			if keys.secondary then
				...
				if keys.secondary:len() > 0 then
					bindComboKeyPress(keys.secondary, callback.callback, keybind.widget)
				end
			end
		end
	end
	...
	for _, callback in ipairs(keybind.callbacks) do
		if callback.type == KEY_UP then
			if keys.primary then
				...
				if keys.primary:len() > 0 then
					unbindComboKeyUp(keys.primary, callback.callback, keybind.widget)
				end
			end
			...
			if keys.secondary then
				...
				if keys.secondary:len() > 0 then
					unbindComboKeyUp(keys.secondary, callback.callback, keybind.widget)
				end
			end
		elseif callback.type == KEY_DOWN then
			if keys.primary then
				...
				if keys.primary:len() > 0 then
					unbindComboKeyDown(keys.primary, callback.callback, keybind.widget)
				end
			end
			...
			if keys.secondary then
				...
				if keys.secondary:len() > 0 then
					unbindComboKeyDown(keys.secondary, callback.callback, keybind.widget)
				end
			end
		elseif callback.type == KEY_PRESS then
			if keys.primary then
				...
				if keys.primary:len() > 0 then
					unbindComboKeyPress(keys.primary, callback.callback, keybind.widget)
				end
			end
			...
			if keys.secondary then
				...
				if keys.secondary:len() > 0 then
					unbindComboKeyPress(keys.secondary, callback.callback, keybind.widget)
				end
			end
		end
	end
...
function Keybind.setPrimaryActionKey(category, action, preset, keyCombo, chatMode)
	...
	if not keys[chatMode] then
		keys[chatMode] = {
			primary = keyCombo,
			secondary = keybind.keys[tonumber(chatMode)].secondary
		}
	end
...
function Keybind.setSecondaryActionKey(category, action, preset, keyCombo, chatMode)
	...
	if not keys[chatMode] then
		keys[chatMode] = {
			primary = keybind.keys[tonumber(chatMode)].primary,
			secondary = keyCombo
		}
	end
...
function Keybind.getKeybindKeys(category, action, chatMode, preset, forceDefault)
	...
	local keys = config and config:getNode(index) or nil
...
function Keybind.bindHotkey(hotkeyId, chatMode)
	...
	if keys.primary then
		...
		if keys.primary:len() > 0 then
			if action == HOTKEY_ACTION.EQUIP or action == HOTKEY_ACTION.USE or action == HOTKEY_ACTION.TEXT or action == HOTKEY_ACTION.TEXT_AUTO then
				bindComboKeyDown(keys.primary, hotkey.callback, gameRootPanel)
			else
				bindComboKeyPress(keys.primary, hotkey.callback, gameRootPanel)
			end
		end
	end
	...
	if keys.secondary then
		...
		if keys.secondary:len() > 0 then
			if action == HOTKEY_ACTION.EQUIP or action == HOTKEY_ACTION.USE or action == HOTKEY_ACTION.TEXT or action == HOTKEY_ACTION.TEXT_AUTO then
				bindComboKeyDown(keys.secondary, hotkey.callback, gameRootPanel)
			else
				bindComboKeyPress(keys.secondary, hotkey.callback, gameRootPanel)
			end
		end
	end
end
...
function Keybind.unbindHotkey(hotkeyId, chatMode)
	...
	if keys.primary then
		...
		if keys.primary:len() > 0 then
			if action == HOTKEY_ACTION.EQUIP or action == HOTKEY_ACTION.USE or action == HOTKEY_ACTION.TEXT or action == HOTKEY_ACTION.TEXT_AUTO then
				unbindComboKeyDown(keys.primary, hotkey.callback, gameRootPanel)
			else
				unbindComboKeyPress(keys.primary, hotkey.callback, gameRootPanel)
			end
		end
	end
	...
	if keys.secondary then
		...
		if keys.secondary:len() > 0 then
			if action == HOTKEY_ACTION.EQUIP or action == HOTKEY_ACTION.USE or action == HOTKEY_ACTION.TEXT or action == HOTKEY_ACTION.TEXT_AUTO then
				unbindComboKeyDown(keys.secondary, hotkey.callback, gameRootPanel)
			else
				unbindComboKeyPress(keys.secondary, hotkey.callback, gameRootPanel)
			end
		end
	end
end
