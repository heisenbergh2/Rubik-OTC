...
local function calcValues(self)
	...
	local noScrollableRange = self.maximum == self.minimum
	local proportion

	if noScrollableRange then
		proportion = 1
	elseif self.virtualChilds > 0 and self.visibleItems then
		proportion = math.min(1, self.visibleItems / self.virtualChilds)
	elseif self.pixelsScroll then
		proportion = pxrange / (range + pxrange)
	else
		proportion = math.min(math.max(self.step, 1), range) / range
	end

	local minSliderSize = 15
	local px = math.max(proportion * pxrange, minSliderSize)
...
local function jumpScrollToTrackPosition(self, mousePos)
	local slider = self:getChildById("sliderButton")
	...
	local range, pxrange, px = calcValues(self)
	...
	local scale = range / denom

	if self.orientation == "vertical" then
		local midY = slider:getY() + slider:getHeight() / 2

		self:setValue(self.value + (mousePos.y - midY) * scale)
	else
		local midX = slider:getX() + slider:getWidth() / 2

		self:setValue(self.value + (mousePos.x - midX) * scale)
	end

	return true
end

local function applyScrollMoveDelta(self, mouseMoved)
	local range, pxrange, px = calcValues(self)
	...
	if self.orientation == "vertical" then
		self:setValue(self.value + mouseMoved.y * scale)
	else
		self:setValue(self.value + mouseMoved.x * scale)
	end
end
...
function UIScrollBar.create()
	...
	scrollbar._trackDragActive = false
	scrollbar.value = 0
...
function UIScrollBar:onMousePress(mousePos, mouseButton)
	...
	local slider = self:getChildById("sliderButton")
	local dec = self:getChildById("decrementButton")
	local inc = self:getChildById("incrementButton")

	if slider and slider:containsPoint(mousePos) then
		return false
	end

	if dec and dec:containsPoint(mousePos) then
		return false
	end

	if inc and inc:containsPoint(mousePos) then
		return false
	end

	if not jumpScrollToTrackPosition(self, mousePos) then
		return false
	end

	self._trackDragActive = true
...
function UIScrollBar:onMouseMove(mousePos, mouseMoved)
	...
	applyScrollMoveDelta(self, mouseMoved)
