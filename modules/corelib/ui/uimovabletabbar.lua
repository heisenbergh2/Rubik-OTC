...
local function updateNavigation(tabBar)
	if tabBar.prevNavigation then
		...
		elseif #tabBar.preTabs > 0 or table.find(tabBar.tabs, tabBar.currentTab) ~= 1 then
			tabBar.prevNavigation:enable()
		else
			tabBar.prevNavigation:disable()
		end
	...
	if tabBar.nextNavigation then
		...
		elseif #tabBar.postTabs > 0 or table.find(tabBar.tabs, tabBar.currentTab) ~= #tabBar.tabs then
			tabBar.nextNavigation:enable()
		else
			tabBar.nextNavigation:disable()
		end
...
local function updateIndexes(tabBar, tab, xoff)
	...
	local dragLeft = xoff
	local dragRight = xoff + tab:getWidth()

	while newIndex > 1 do
		local leftTab = tabs[newIndex - 1]
	...
	while newIndex < #tabs do
		local rightTab = tabs[newIndex + 1]

		if rightTab == tab then
			break
		end
...
function UIMoveableTabBar:endBatchRemove()
	...
	self._batchRemoveDepth = depth - 1

	if self._batchRemoveDepth == 0 then
		updateTabs(self)
	end
end
...
function UIMoveableTabBar:removeTab(tab)
	...
	if (self._batchRemoveDepth or 0) == 0 then
		updateTabs(self)
	end
end
...
function UIMoveableTabBar:setDropTarget(widget, callback)
	...
	if widget then
		function widget.onDrop(target, draggedWidget, mousePos)
