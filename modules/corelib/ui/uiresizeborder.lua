-- chunkname: @/corelib/ui/uiresizeborder.lua
...
function UIResizeBorder.create()
	...
	resizeborder.maximum = 2160
	resizeborder.disableHoverFade = false
...
function UIResizeBorder:onHoverChange(hovered)
	if hovered then
		...
		if not self:isPressed() and not self.disableHoverFade then
			g_effects.fadeIn(self)
		end
	elseif not self:isPressed() and self.hovering then
		g_mouse.popCursor(self.cursortype)

		if not self.disableHoverFade then
...
function UIResizeBorder:onMouseRelease(mousePos, mouseButton)
	...
	if parent and parent.getClassName and parent:getClassName() == "UIMiniWindow" and not parent:isOn() then
		parent:setSettings({
...
function UIResizeBorder:onStyleApply(styleName, styleNode)
	for name, value in pairs(styleNode) do
		...
		elseif name == "disable-hover-fade" then
			self.disableHoverFade = toboolean(value)
		end
	end
end
