...
function requestHighscore(action, page, tag)
	if highscoreRequestPending then
		highscoreDebugLog(string.format("REQ ignorada (async servidor): %s page=%d", tag or "?", page))
...
local function fillHighscoreList(highscores)
	...
	for index = 1, ENTRIES_PER_PAGE do
		...
		if character then
			...
			widget.vocation:setText(g_game.getVocationName(character[4]))
...
function onProcessHighscores(worlds, selectedWorld, worldType, battlEye, vocations, categories, selectedVocation, selectedCategory, page, totalPages, highscores, entriesTs)
	highscoreRequestPending = false
	...
	highscoreDebugLog(string.format("RSP apos REQ #%s tag=%s | page=%d/%d entries=%d/%d srvWorld=\"%s\" srvWType=%d srvVoc=0x%X srvCat=%d | playerNaLista=%s | %s", req and tostring(req.seq) or "?", req and req.tag or "?", page, totalPages, #highscores, ENTRIES_PER_PAGE, selectedWorld or "", worldType, selectedVocation, selectedCategory, tostring(hasPlayer), getFilterSnapshot()))
	highscoreDebugLog("RSP entradas: " .. summarizeHighscoreEntries(highscores))

	if req and req.action == 0 and req.page and page ~= req.page then
		highscoreDebugLog(string.format("ALERTA page divergente: REQ #%d pediu page=%d mas servidor respondeu page=%d ? verifique parse do pacote no servidor", req.seq, req.page, page))
	end

	if req and req.action == 0 and #highscores > 0 and #highscores < ENTRIES_PER_PAGE then
		highscoreDebugLog(string.format("servidor enviou %d/%d entradas ? corrija entriesPerPage no parse do servidor (use %d)", #highscores, ENTRIES_PER_PAGE, ENTRIES_PER_PAGE))
	end

	cachedBattlEye = battlEye

	if not isFilled then
		highscoreDebugLog("RSP setupFilterOptions (primeiro preenchimento)")
