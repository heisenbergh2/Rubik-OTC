-- chunkname: @/modulelib/ext/parseevent.lua
...
local function parseEvents(el, widget, eventName, callStr, controller)
...
local function onCreateWidget(el, widget, controller)
	local function getFncSet(exp)
...
local function createRadioGroup(el, groups, controller)
...
local function afterLoadElement(el)
	if el.name == "hr" and el.widget:hasAnchoredLayout() then
